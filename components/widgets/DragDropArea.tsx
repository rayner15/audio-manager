...
export const DragDropArea: React.FC<DragDropAreaProps> = ({
  ...
  const handleFileDrop = useCallback(
    (e: React.DragEvent) => {
      ...
      if (hasInvalidFile) {
        alert(
          "Some files were skipped. Please ensure all files are valid audio files (MP3, WAV, or M4A) and less than 25MB."
        );
      }
    ...
    <>
      ...
      >
        ...
        <p className="text-xs text-blue-600 mt-4 font-medium">
          Supports MP3 and WAV (max {MAX_FILE_SIZE / 1024 / 1024}MB each)
        </p>
      </div>
